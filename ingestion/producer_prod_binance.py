...
async def coinbase_stream(topic_name: str, bootstrap_servers: str = "localhost:9092"):
    ...
    async with websockets.connect(url) as websocket:
        print("Subscribed to Binance stream for BTC-USD")
        
        try:
            while True:
                message = await websocket.recv()
                data = json.loads(message)
                if data.get('E') is not None and data.get('p') is not None:
                    # Send to Kafka
                    producer.send(
                        topic=topic_name,
                        key=str(data.get('E')).encode('utf-8'),
                        value=data
                        )
                
                # Print a small log to know it works
                print(f"Message envoyé au topic {topic_name}: {data}")
...
if __name__ == "__main__":
    ...
    topic_name = "binance.btc.usd.trades"
    num_partitions = 3
